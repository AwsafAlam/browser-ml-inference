...
const model = './classifier_int8.onnx'
...
// 	let maxValue = arr[0] // Start with the first element as the max value

// 	// Iterate over the array starting from the second element
// 	for (let i = 1; i < arr.length; i++) {
// 		if (arr[i] > maxValue) {
// 			// Compare each element with the current max value
// 			maxValue = arr[i] // Update the max value if a new one is found
...
async function lm_inference(text) {
	try {
		...
		console.log('Encoded ID:', encoded_ids)
		...
		const result_list = []
		result_list.push(['Emotion', 'Score'])
